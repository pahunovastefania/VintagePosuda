...
Первый администратор задаётся в `src/VintagePosuda.Web/appsettings.json` в секции `AdminUser`.

| Логин | Пароль | Роль |
| --- | --- | --- |
| `admin@vintage.local` | `Admin123` | `Admin` |

Админские ссылки не показываются обычному пользователю. После входа администратор видит переходы к каталогу, отчётам и форме добавления товара.
